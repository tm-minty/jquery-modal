(function($){
	$.fn.modal = function(){
		var args = Array.prototype.slice.call(arguments),
			...
			methods = {
				alignCenter: function(){
					...
					if( !isMobile && windowHeight >= height && windowWidth >= width ){
				...
				decorator: function(){
					var begin = '<div id="jquery-modal-overlay"><div id="jquery-modal-overlay-popup">',
					    end = '<a id="jquery-modal-overlay-popup-close">' + options.closeText + '</a></div></div>';
					return [begin, end];
				},
				hide: function(){
				...
				resize: function(e){
					methods.alignCenter();
					if( !isMobile && $(window).height() >= options.height ){
				...
				show: function(){
					...
					if( overlay.length > 0 ){
					 	if( $(document.body).width() > overlay.width() )
					 		overlay.width($(document.body).width());

					 	if( $(document.body).height() > overlay.height() )
							overlay.height($(document.body).height());

						$('#ruru-overlay-popup-close').bind('click', methods.hide);
						overlay.bind('click', methods.hide)
		...
		return $(this).each(function(){
			...
			if( nodeName == 'A' ){
				$(this).attr('onlick', 'javascript: return false;').bind('click', function(){ methods.iframe($(this).attr('href')); return false; });
				...
				if( options.show != 'auto' ){
					modal.hide();
					$(options.show).bind( options.showOn, function(){ methods.show(); options.onComplete(); } );
				}else{
					methods.show();
					options.onComplete();
				}
